...
class WebhookHandler:
    ...
    def __init__(self, bot: "ForgejoBot") -> None:
        ...
        self.msgtype = (
            MessageType(bot.config["message_options.msgtype"]) or MessageType.NOTICE
        )
        PendingAggregation.timeout = int(
            bot.config["message_options.aggregation_timeout"]
        )
    ...
    def reload_config(self) -> None:
        ...
        self.msgtype = (
            MessageType(self.bot.config["message_options.msgtype"])
            or MessageType.NOTICE
        )
        PendingAggregation.timeout = int(
            self.bot.config["message_options.aggregation_timeout"]
        )
    ...
    async def __call__(
        ...
        if evt_type == EventType.PING:
            ...
            await self.bot.webhook_manager.set_forgejo_id(info, evt.hook_id)
            ...
            if evt.action in (RepositoryAction.TRANSFERRED, RepositoryAction.RENAMED):
                action = (
                    "transfer"
                    if evt.action == RepositoryAction.TRANSFERRED
                    else "rename"
                )
                ...
                await self.bot.webhook_manager.delete(info.id)
        elif evt_type == EventType.PUSH and (
            evt.size is None or evt.distinct_size is None
        ):
            assert isinstance(evt, PushEvent)
            evt.size = len(evt.commits)
            evt.distinct_size = len(
                [commit for commit in evt.commits if commit.distinct is not False]
            )
            ...
            try:
                ...
                return
            reaction["xyz.maubot.forgejo.webhook"] = {
        ...
        if PendingAggregation.timeout < 0:
            ...
            event_id = await self.send_message(
                evt_type, evt, info.room_id, {delivery_id}
            )
        ...
        asyncio.ensure_future(
            PendingAggregation(self, evt_type, evt, delivery_id, info).start()
        )
    ...
    async def send_message(
        ...
        content["xyz.maubot.forgejo.webhook"] = {
