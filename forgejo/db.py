...
class WebhookInfo:
    ...
    def from_row(cls, row: Record | None) -> Optional["WebhookInfo"]:
        ...
        forgejo_id = row[
            "github_id"
        ]  # DB column still uses github_id for migration compat
        secret = row["secret"]
        return cls(
            ...
            user_id=user_id,
            room_id=room_id,
            forgejo_id=forgejo_id,
            secret=secret,
        )

    def __str__(self) -> str:
        return (
            f"webhook {self.id!s} (FG{self.forgejo_id}) from {self.repo} to {self.room_id}"
...
class DBManager:
    ...
    async def get_clients(self) -> list[Client]:
        rows = await self.db.fetch(
            "SELECT user_id, token, refresh_token, expiry FROM client"
        )
    ...
    async def put_client(
        ...
        await self.db.execute(
            """
            INSERT INTO client (user_id, token, refresh_token, expiry) VALUES ($1, $2, $3, $4)
            ON CONFLICT (user_id) DO UPDATE SET token = excluded.token, refresh_token = excluded.refresh_token, expiry = excluded.expiry
            """,
            user_id,
            token,
            refresh_token,
